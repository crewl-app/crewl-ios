...
class CountryListData {
	// MARK: Country Bottom Sheet Data.
	static var countries: [Country] {
		...
		for (dialCode, countryCode) in countryDictionary {
			...
			if (!name.isEmpty) {
				countries.append(Country(name: name, dialogCode: dialCode, countryCode: countryCode, flag: flag))
			}
		}
	...
	private static func getCountryName(dialCode: String) -> String {
		let current = Locale(identifier: "en_US")

		return current.localizedString(forRegionCode: dialCode) ?? ""
	}

	private static func getCountryFlag(dialCode: String) -> String {
	...
	private static var countryDictionary: [String: String] {
