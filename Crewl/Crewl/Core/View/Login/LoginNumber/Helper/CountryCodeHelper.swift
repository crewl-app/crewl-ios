...
func getCountries() -> [Country] {
    let countryDictionary = getCountryDictionary()
    var listOfCountries: [Country] = []
    
    for (dialCode, countryCode) in countryDictionary {
        let flag = getCountryFlag(dialCode: dialCode)
        let name = getCountryName(dialCode: dialCode)
        
        if (!name.isEmpty) {
            listOfCountries.append(Country(name: name, dialogCode: dialCode, countryCode: countryCode, flag: flag))
        }
    }
    
    return listOfCountries.sorted {
        $0.name < $1.name
    }
}


private func getCountryName(dialCode: String) -> String {
    let current = Locale(identifier: "en_US")
    return current.localizedString(forRegionCode: dialCode) ?? ""
}


private func getCountryFlag(dialCode: String) -> String {
    let base : UInt32 = 127397
    var flag = ""
    for v in dialCode.unicodeScalars {
...
private func getCountryDictionary() -> [String: String] {
