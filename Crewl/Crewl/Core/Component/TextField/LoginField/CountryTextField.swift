        ...
        ZStack {
            countryBackground()
                .frame(width: 70,
                       height: 40)
            HStack {
                Text(countryCode.isEmpty ? "🇹🇷 +90" : "\(countryFlag) +\(countryCode)")
            }
            .frame(width: 70,
                   height: 40)
...
struct CustomCountryTextField_Previews: PreviewProvider {
    static var previews: some View {
        ZStack {
            ...
            //                .ignoresSafeArea()
            //            CountryTextField(y: CGFloat(), countryCode: "", countryFlag: "")
            CountryCodes(countryCode: .constant(""),
                         countryFlag: .constant(""),
                         isClickedCountry: .constant(false))
        }
    }
}

private struct countryBackground: View {
    var body: some View {
